...
try:
  import pt_autograph._ptag
  # These are basically direct calls into 
  # pytorch_scope.h
  from pt_autograph._ptag import (
    _ptag_add_frontend_attribute,
    _ptag_add_frontend_attributes,
    _ptag_remove_frontend_attribute,
    _ptag_remove_frontend_attributes,
  )
  _FAS_SUPPORTED = True
except:
  pass
...
def ir_scope(scope_text, enabled=True and _FAS_SUPPORTED):
  ...
  """
  if enabled:
    pt_autograph._ptag._ptag_push_ir_scope(scope_text)
  try:
    yield
  finally:
    if enabled:
      pt_autograph._ptag._ptag_pop_ir_scope()
...
def frontend_attribute_scope(key=None, value=None, values=None, enabled=_FAS_SUPPORTED):
  ...
  if enabled:
    if values:
      ...
      emit_keys = _ptag_add_frontend_attributes(
        key,
        value
      )
    else:
      emit_key = _ptag_add_frontend_attribute(
        key,
        value
      )
  try:
