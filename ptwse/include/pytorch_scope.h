 ...
 * under the License.
 */
#ifndef PYTORCH_PTWSE_SCOPE_H_INCLUDED
#define PYTORCH_PTWSE_SCOPE_H_INCLUDED
...
#ifndef PYTORCH_PTWSE_NO_FA_SETTER
#include "tensorflow/compiler/xla/client/xla_builder.h"
#endif


#define PYTORCH_PTWSE_PUBLIC __attribute__((visibility("default")))

#define PYTORCH_PTWSE_THREAD_LOCAL thread_local

namespace pytorch_ptwse {
...
class FrontendAttributePusher {
 ...
 public:
  ...
  static void Reset() {
    if (!g_frontend_attribute_context.attributes.empty()) {
      std::cerr << "[PTWSE SCOPE WARNING]: Not all frontend attributes were popped"
                << std::endl;
    }
 ...
 private:
  ...
  static void InitForceEnable() {
    const char *s = ::getenv("PYTORCH_PTWSE_FORCE_ENABLE_FATTR");
    if (!s) {
      return;
    }
    if (*s == '1') {
  ...
  static PYTORCH_PTWSE_PUBLIC PYTORCH_PTWSE_THREAD_LOCAL FrontendAttributeContext g_frontend_attribute_context;
  static signed int PYTORCH_PTWSE_PUBLIC PYTORCH_PTWSE_THREAD_LOCAL enable_count_;
  static bool PYTORCH_PTWSE_PUBLIC PYTORCH_PTWSE_THREAD_LOCAL force_enable_;
};
...
#define PYTORCH_PTWSE_INSTANTIATE_PARTITIONS(force_enable) \
    namespace pytorch_ptwse { \
        PYTORCH_PTWSE_PUBLIC PYTORCH_PTWSE_THREAD_LOCAL FrontendAttributePusher::FrontendAttributeContext \
        FrontendAttributePusher::g_frontend_attribute_context;                                    \
        PYTORCH_PTWSE_PUBLIC PYTORCH_PTWSE_THREAD_LOCAL signed int FrontendAttributePusher::enable_count_{0};     \
        PYTORCH_PTWSE_PUBLIC PYTORCH_PTWSE_THREAD_LOCAL bool FrontendAttributePusher::force_enable_{force_enable};      \
  }  /* end of pytorch_ptwse namespace */


#define PYTORCH_PTWSE_ENABLE_PARITIONS_MACRO

#ifdef PYTORCH_PTWSE_ENABLE_PARITIONS_MACRO
#define PYTORCH_PTWSE_DECLARE_PARTITION()                    \
  pytorch_ptwse::FrontendAttributePusher fattr(    \
      pytorch_ptwse::PartitionScope::PartitionMatchName(\
        pytorch_ptwse::PartitionScope::infer_direction(__FUNCTION__)), \
        pytorch_ptwse::PartitionScope::MakePartitionName(__FUNCTION__) \
      )

#define PYTORCH_PTWSE_DECLARE_PARTITION_FWD()                    \
  pytorch_ptwse::FrontendAttributePusher fattr(    \
      pytorch_ptwse::PartitionScope::PartitionMatchName(\
        pytorch_ptwse::Direction::FWD), \
        pytorch_ptwse::PartitionScope::MakePartitionName(__FUNCTION__) \
      )

#define PYTORCH_PTWSE_DECLARE_PARTITION_CLASS_FWD()                    \
  pytorch_ptwse::FrontendAttributePusher fattr(    \
      pytorch_ptwse::PartitionScope::PartitionMatchName(\
        pytorch_ptwse::Direction::BWD), \
        pytorch_ptwse::PartitionScope::MakePartitionName(pytorch_ptwse::PartitionScope::method_name(__PRETTY_FUNCTION__)) \
      )

#define PYTORCH_PTWSE_DECLARE_PARTITION_BYNAME_FWD(__name$)                    \
  pytorch_ptwse::FrontendAttributePusher fattr(    \
      pytorch_ptwse::PartitionScope::PartitionMatchName(\
        pytorch_ptwse::Direction::FWD), \
        pytorch_ptwse::PartitionScope::MakePartitionName(__name$) \
      )

#define PYTORCH_PTWSE_DECLARE_PARTITION_BWD()                    \
  pytorch_ptwse::FrontendAttributePusher fattr(    \
      pytorch_ptwse::PartitionScope::PartitionMatchName(\
        pytorch_ptwse::Direction::BWD), \
        pytorch_ptwse::PartitionScope::MakePartitionName(__FUNCTION__) \
...
#define PYTORCH_PTWSE_DECLARE_PARTITION_CLASS_BWD()                    \
  pytorch_ptwse::FrontendAttributePusher fattr(    \
      pytorch_ptwse::PartitionScope::PartitionMatchName(\
        pytorch_ptwse::Direction::BWD), \
        pytorch_ptwse::PartitionScope::MakePartitionName(\
          pytorch_ptwse::PartitionScope::method_name(__PRETTY_FUNCTION__) \
      ))

#define PYTORCH_PTWSE_DECLARE_PARTITION_BYNAME_BWD(__name$)                    \
  pytorch_ptwse::FrontendAttributePusher fattr(    \
      pytorch_ptwse::PartitionScope::PartitionMatchName(\
        pytorch_ptwse::Direction::BWD), \
        pytorch_ptwse::PartitionScope::MakePartitionName(__name$), \
...
#else
#define PYTORCH_PTWSE_DECLARE_PARTITION() ((void)0)
#define PYTORCH_PTWSE_DECLARE_XLA_PARTITION() ((void)0)extra_attributes)
#endif
...
#define DECLARE_XLA_PARTITION(__builder$)                    \
  pytorch_ptwse::FrontendAttributeSetter fattr(__builder$,    \
    {{pytorch_ptwse::PartitionScope::PartitionMatchName(\
        !pytorch_ptwse::PartitionScope::is_backward_name(__FUNCTION__), \
        PartitionScope::MATCHED_XLA), \
    pytorch_ptwse::PartitionScope::MakePartitionName(__FUNCTION__)}} \
  )

#endif // PYTORCH_PTWSE_NO_FA_SETTER

}  // end of pytorch_ptwse namespace

#endif  // PYTORCH_PTWSE_SCOPE_H_INCLUDED
